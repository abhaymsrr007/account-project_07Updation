    ...
    $scope.fundDathhhhh = function () {
      ...
      $http
        ...
        .then(
          ...
          function (error) {
            // console.log("erroe has occured in post data")
            toastr.error("An error occurred while saving data.", "Error");
          }
        );
      //   }else{
         ...
         if(costCentreId){
          $http.delete(path + `/api/v1/account/costCenter/${costCentreId}`, config)
            .then(function (response) {
              ...
              function (error) {
                toastr.error('Costcenter id is in used! You Can Not deleted', 'error',error);
                $('#deleteConfirmationModal2').modal('hide');
              }
            );
         }else{
    ...
    $scope.deleteCostIdOfBudget = function (planNo, planFinYear, planTypeCode, costCentreId, allocationId) {
      $http
        ...
        .then(
          ...
          function (error) {
            toastr.error("Data Not Deleted budget !", "error",{timeOut: 3000});
            $('#deleteConfirmationModal').modal('hide');         
          }
        );
    };
    ...
    $scope.deleteCostIdOfFund = function (planNo, planFinYear, planTypeCode, costCentreId, transferSlNo) {
      $http
        ...
        .then(
          ...
          function (error) {
            toastr.error("Data Not Deleted !", "error",{timeOut: 3000}); 
            $('#deleteConfirmationModal1').modal('hide');
          }
        );
    };
