  ...
  function ($scope, $http, $timeout, $window, $rootScope) {
    ...
    $scope.setpayFromTypeCode = function (payFromTypeCode) {
      ...
      if (payFromTypeCode === "E") {
        $scope.payFromTypeCodes = $scope.cPvData.filter(
    ...
    $scope.handleLink = function (p){$rootScope.handleLink(p);$rootScope.dataReset();};
    $scope.isDisable = 0;
    ...
    $scope.getdata = function () {
      $http.get(path + "/api/v1/account/cpv", config).then(function (response) {
    ...
    $scope.delete = function (cashPvId, finyear) {
      $http
        .delete(path + `/api/v1/account/cpv/${cashPvId}/${finyear}`, config)
        .then(
    ...
    $scope.edit = function (cashPvId, finYear) {
      const nObject = {
        cashPvId: cashPvId,
        finYear: finYear,
      };
    ...
    $scope.searchFields = function (
      ...
      console.log("cashPayVoucherDto", cashPayVoucherDto);
      $http
        .post(path + "/api/v1/account/cpv/search", cashPayVoucherDto, config)
        .then(
          function successCallback(response) {
            $scope.cPvData = response.data;
            $scope.cPvData = response.data;
