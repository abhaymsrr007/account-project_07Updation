app.controller(
  ...
  function ($scope, $http, $timeout, $window, $rootScope, $filter, $location) {
    ...
    $scope.post = function (accountCode, monthYear) {
      var monthyear = monthYear.toString().split("-");
      const url =
        "http://192.168.88.169:9099/api/v1/sub-ledger/?year=" +
        monthyear[0] +
        "&month=" +
        monthyear[1] +
        "&accountCode=" +
      ...
      console.log(url);
      console.log(
        "http://192.168.88.169:9099/api/v1/sub-ledger/?year=2024&month=02&accountCode=U55793917"
      )
    ...
    $scope.getSubLData = function () {
      $http
        .get("http://192.168.88.169:9099/api/v1/sub-ledger/page", config)
        .then(
          function successCallback(response) {
    ...
    $scope.exportTableToExcel = function () {
      ...
      if (table) {
        ...
        XLSX.writeFile(wb, "Sub-Ledger.xlsx");
      }
    };
