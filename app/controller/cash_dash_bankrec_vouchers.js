  ...
  function ($scope, $http, $timeout, $window, $rootScope) {
    ...
    $scope.setpayFromTypeCode = function (payFromTypeCode) {
      ...
      if (payFromTypeCode === "E") {
        $scope.payFromTypeCodes = $scope.bRvData.filter(
    ...
    $scope.showDetails = function (n) {
      ...
      $rootScope.handleLinkWData("bank-receipt-voucher", nObject);
    };
    ...
    $scope.getdata = function () {
      $http.get(path + "/api/v1/account/brv", config).then(function (response) {
        $scope.bRvData = response.data;
        console.table("$scope.bRvData", response.data);
      });
    };
    $scope.getdata();
    $scope.closeDetail = () => {
    ...
    $scope.delete = function (bankRvId, finyear) {
      $http
        .delete(path + `/api/v1/account/brv/${bankRvId}/${finyear}`, config)
        .then(
    ...
    $scope.edit = function (bankRvId, finYear) {
      const nObject = {
        bankRvId: bankRvId,
        finYear: finYear,
      };
    ...
    $scope.searchFields = function (
      ...
      var bankReceiptVoucherDto = {
        bankRvId: keyword1 || null,
        bankRvIdDate: keyword2 ? idDate : null,
      ...
      $http
        .post(
          path + "/api/v1/account/brv/search",
          bankReceiptVoucherDto,
          config
        )
        .then(
          function successCallback(response) {
